...
exports.verifyFuncionario= function (req, res, next){
    if (req.body.user_group_id == 2)
        ...
        return res.status(500).send({ auth: false, message: 'Falha de autenticação. Apenas Funcionarios permitidos' });
};
...
exports.verifyVIP= function (req, res, next){
    ...
    if (group == 2 || group == 3)
        ...
        return res.status(500).send({ auth: false, message: 'Falha de autenticação.Apenas Funcionarios ou VIPS permitidos' });
};